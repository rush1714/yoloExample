  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        ec2: true,
        commands: [
          { target: '3-brand-s3-workflow-ec2-train', title: '一键上传并训练 S3 数据', description: '依次上传 labels/EC2 manifest/YAML、让 EC2 从 S3 下载图片并启动训练；默认 dry-run。', params: ['S3_DATASET_NAME', 'S3_LABEL_NAME', 'S3_DATASET_ROOT', 'EC2_HOST', 'EC2_USER', 'EC2_KEY', 'EC2_PORT', 'EC2_PROJECT_ROOT', 'EC2_BASE_MODEL', 'EC2_TRAIN_EPOCHS', 'EC2_TRAIN_IMGSZ', 'EC2_TRAIN_BATCH', 'EC2_TRAIN_DEVICE', 'EC2_RUN_NAME', 'EC2_EXECUTE'] },
          { target: 'brand-s3-ec2-upload-manifest', title: '上传 S3 manifest 到 EC2', description: '上传 labels、EC2 图片 manifest JSON/CSV 和 YAML，不上传图片大文件。', params: ['S3_DATASET_NAME', 'S3_LABEL_NAME', 'S3_DATASET_ROOT', 'S3_EC2_IMAGE_MANIFEST_JSON', 'S3_EC2_IMAGE_MANIFEST_CSV', 'S3_EC2_REMOTE_MANIFEST_JSON', 'S3_EC2_REMOTE_MANIFEST_CSV', 'EC2_HOST', 'EC2_USER', 'EC2_KEY', 'EC2_RUN_NAME', 'EC2_EXECUTE'] },
          { target: 'brand-s3-ec2-download-images', title: 'EC2 下载 S3 训练图片', description: '在 EC2 上读取 ec2_image_manifest.json 从 S3 下载训练图片，默认 dry-run。', params: ['S3_DATASET_NAME', 'S3_EC2_REMOTE_MANIFEST_JSON', 'EC2_HOST', 'EC2_USER', 'EC2_KEY', 'EC2_PROJECT_ROOT', 'EC2_EXECUTE'] },
          { target: 'brand-s3-ec2-train', title: 'EC2 训练 S3 数据', description: 'EC2 先读取 ec2_image_manifest.json 下载 S3 图片再训练，训练规模由 EC2_* 显式参数控制。', params: ['S3_DATASET_NAME', 'S3_LABEL_NAME', 'S3_EC2_REMOTE_MANIFEST_JSON', 'EC2_HOST', 'EC2_USER', 'EC2_KEY', 'EC2_BASE_MODEL', 'EC2_TRAIN_EPOCHS', 'EC2_TRAIN_IMGSZ', 'EC2_TRAIN_BATCH', 'EC2_TRAIN_DEVICE', 'EC2_RUN_NAME', 'EC2_EXECUTE'] },
...
const PARAM_DEFINITIONS = {
  ...
  S3_PREFIX: { label: 'S3 业务前缀', defaultValue: 'local_dataset', type: 'text', help: '只填业务子目录；实际上传会自动使用 yolo-training/<S3_PREFIX>。' },
  S3_REGION: { label: 'S3 区域', defaultValue: 'af-south-1', type: 'text' },
  S3_PROFILE: { label: 'AWS Profile', defaultValue: 'smdp-yolo', type: 'text' },
  S3_ENDPOINT_URL: { label: 'S3 Endpoint URL', defaultValue: '', type: 'text', help: '兼容 S3 服务端点，AWS S3 通常留空。' },
  S3_PUBLIC_BASE_URL: { label: 'S3 公开 Base URL', defaultValue: '', type: 'text', help: '公开桶或 CDN 前缀；留空时由桶和区域推导。' },
  ...
  S3_UPLOAD_WORKERS: { label: 'S3 上传并发', defaultValue: '8', type: 'number', help: '并发上传线程数；断点续传会跳过已成功上传的图片。' },
  S3_MANIFEST_JSON: { label: 'S3 图片清单 JSON', defaultValue: '', type: 'text' },
  S3_IMAGE_URL_MODE: { label: 'LS 图片地址模式', defaultValue: 'nginx', type: 'select', options: ['nginx', 'proxy', 'https', 's3'] },
  ...
  S3_NGINX_MODE: { label: 'Nginx 回源模式', defaultValue: 'presign', type: 'select', options: ['presign', 'direct'], help: '私有桶用 presign；公开桶可用 direct。' },
  S3_NGINX_PRESIGN_EXPIRES: { label: '预签名秒数', defaultValue: '604800', type: 'number', help: '默认 7 天，过期前可刷新 Nginx 图片代理。' },
  S3_NGINX_CONF: { label: 'Nginx 配置路径', defaultValue: '', type: 'text', help: '留空时使用 .tmp/s3-nginx/<数据集>/nginx.conf。' },
  S3_NGINX_MAP: { label: 'Nginx map 路径', defaultValue: '', type: 'text', help: '留空时使用 .tmp/s3-nginx/<数据集>/s3_image_map.conf。' },
  S3_NGINX_CACHE_DIR: { label: 'Nginx 缓存目录', defaultValue: '', type: 'text', help: '留空时使用 .tmp/s3-nginx/<数据集>/cache。' },
  S3_NGINX_PID: { label: 'Nginx PID 路径', defaultValue: '', type: 'text', help: '留空时使用 .tmp/s3-nginx/<数据集>/nginx.pid。' },
  ...
  S3_LS_EXPORT_PATH: { label: 'S3 LS 导出 JSON', defaultValue: '', type: 'text', help: '留空时使用 datasets/s3/<数据集>/label_studio/exports/label_studio_export.json。' },
  S3_DATA_YAML: { label: 'S3 YOLO YAML', defaultValue: '', type: 'text', help: '留空时使用 config/generated/s3_<数据集>.yaml。' },
  S3_EC2_IMAGE_MANIFEST_JSON: { label: '本地 EC2 图片清单 JSON', defaultValue: '', type: 'text', help: '标注转 YOLO 后生成，默认 metadata/ec2_image_manifest.json；不同于上传清单 s3_images.json。' },
  S3_EC2_IMAGE_MANIFEST_CSV: { label: '本地 EC2 图片清单 CSV', defaultValue: '', type: 'text', help: '标注转 YOLO 后生成，默认 metadata/ec2_image_manifest.csv；会随 JSON 一起上传到 EC2。' },
  S3_EC2_REMOTE_MANIFEST_JSON: { label: '远端 EC2 图片清单 JSON', defaultValue: '', type: 'text', help: 'EC2 上读取的下载清单路径，默认 datasets/s3/<数据集>/metadata/ec2_image_manifest.json。' },
  S3_EC2_REMOTE_MANIFEST_CSV: { label: '远端 EC2 图片清单 CSV', defaultValue: '', type: 'text', help: '上传到 EC2 的 CSV 清单路径，默认 datasets/s3/<数据集>/metadata/ec2_image_manifest.csv。' },
