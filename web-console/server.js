...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      {
        target: 'diaper-merge-ls-projects',
        title: '合并多个 LS 项目有效标注',
        description: '按 LS_PROJECT_IDS 顺序导出多个项目，仅保留有有效框的任务并去重合并。',
        params: ['DIAPER_COUNTRY', 'DIAPER_VERSION', 'DIAPER_LABEL_NAME', 'LS_PROJECT_IDS', 'DIAPER_MERGED_LS_EXPORT_PATH', 'DIAPER_MERGE_REPORT'],
      },
      {
        target: 'diaper-merge-ls-projects-to-yolo',
        title: '合并多个 LS 项目并生成 YOLO',
        description: '导出多个 LS 项目，合并有有效框的任务，然后转换为 YOLO 训练集。',
        params: ['DIAPER_COUNTRY', 'DIAPER_VERSION', 'DIAPER_LABEL_NAME', 'LS_PROJECT_IDS', 'LS_TO_YOLO_CLEAR', 'LS_TO_YOLO_SKIP_EMPTY', 'DIAPER_MERGED_LS_EXPORT_PATH', 'DIAPER_MERGE_REPORT'],
      },
      {
        target: 'diaper-yaml',
        title: '生成纸尿裤 YAML',
      ...
      {
        target: '02-diaper-ec2-upload-data',
        title: '02. 上传纸尿裤数据到 EC2',
        description: '上传本地纸尿裤 YOLO 数据集和 YAML 到 EC2；默认 dry-run，EC2_EXECUTE=1 才实际上传。',
        params: ['DIAPER_COUNTRY', 'DIAPER_VERSION', 'DIAPER_LABEL_NAME', 'DIAPER_DATASET_ROOT', 'DIAPER_DATA_YAML', 'EC2_HOST', 'EC2_USER', 'EC2_KEY', 'EC2_PORT', 'EC2_PROJECT_ROOT', 'EC2_REMOTE_DATA_YAML', 'EC2_EXECUTE'],
      },
      {
