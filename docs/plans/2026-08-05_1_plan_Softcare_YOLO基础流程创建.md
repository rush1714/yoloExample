...
- 2026-08-06 更新：改进 OCR 品牌匹配规则。`scripts/ocr/filter_brand_candidates.py` 不再使用 `partial_ratio * confidence` 简单加权，新增短文本过滤、品牌覆盖比例约束和组合相似度；`KLEESOFT` 品牌库补充 `Keeson`、`Kleeson`、`Keesoft`、`KEESOTE`、`NEESOTE` 等 OCR 常见误识别别名。字符串回归测试确认：`Viva` 不再误命中 `LAVITA`，`K` 不再误命中 `KLEESOFT`，`Keeson` 可作为 `KLEESOFT` 候选。
...
- 2026-08-10 更新：常规 OCR 已支持多线程并行识别。`scripts/ocr/filter_brand_candidates.py` 新增 `--workers`，Makefile 新增 `OCR_WORKERS`，默认 `rapidocr + 4 workers`，报告和 `ocr_candidates.txt` 输出格式保持不变。
- 2026-08-10 更新：新增 Ollama 本地视觉大模型 OCR 分支。新增 `scripts/ocr/filter_brand_candidates_llm.py`、`make step-2-ocr-llm` 和 `make workflow-to-ls-llm`；本机模型检查发现 `gemma3:12b`、`qwen3.6:latest`、`minicpm-v:latest` 均支持 vision，其中样例图上 `gemma3:12b` 可稳定输出 `Softcare, Colgate, Pepsodent, Longrich`，因此作为默认 LLM OCR 模型。
