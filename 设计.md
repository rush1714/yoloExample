...
当前类别定义已统一迁移到可持久化的 `config/label_categories.json`。历史品牌集合仍保留为 `LABEL_SET=brands`，并兼容旧 `config/brand_keywords.json` / `BRAND=<品牌>` 命令；新的通用流程使用 `LABEL_SET=<类别列表>` 与 `LABELS=<类别1,类别2>` 选择单个或多个类别，空值或 `all` 表示选择当前列表下所有启用类别。控制台“类别管理”可维护类别列表，修改后写回 `config/label_categories.json` 并可提交 Git。
