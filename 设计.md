...
- S3 上传脚本：`scripts/s3/upload_images_to_s3.py`，输出 `datasets/s3/<S3_DATASET_NAME>/metadata/s3_images.json|csv` 和 `s3_download_urls.txt`；用户传入的 `S3_PREFIX` 是业务子目录，实际 S3 key 会自动归入 `yolo-training/<S3_PREFIX>/...`，若已传 `yolo-training/...` 则不会重复拼接。
