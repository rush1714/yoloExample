...
- 使用左侧菜单展示品牌多类别流程、Label Studio、训练推理、纸尿裤大类/EC2、维护工具和图片浏览等功能大类。
- 命令类功能在左侧展开二级子菜单；右侧内容区只展示当前选中的单个命令页面，包括说明、常用参数输入框、命令预览、复制和执行按钮，未填写参数继续使用 Makefile 默认值。
...
- 本地 Web 控制台左侧菜单提供“本地目录导入 / 标注”分组，可填写同样参数并执行。
...
models/ec2/local/<LOCAL_DATASET_NAME>/<profile>/best.pt
artifacts/local/<LOCAL_DATASET_NAME>/<profile>/
```

对应 Make 入口：

- `make local-dir-ec2-upload-data`：上传本地目录 YOLO 数据集和 YAML 到 EC2。
- `make local-dir-ec2-train-smoke` / `make local-dir-ec2-train-baseline` / `make local-dir-ec2-train-improve`：按不同档位训练。
...
这些 EC2 命令默认 dry-run，只有显式传 `EC2_EXECUTE=1` 才会真正连接远端执行。
