...
- 数据默认按国家、版本和类别隔离：`datasets/<来源>/<国家>/<版本>/<数据集>/`，例如 `datasets/local/GH/v2026-09-18/allround_purple/` 或 `datasets/s3/GH/v2026-09-18/general_all/`。
- 类别列表：`config/label_categories.json`，通过 `LABEL_SET` 选择列表，通过 `LABELS` 多选类别。
- 原图目录：`<数据集>/raw/images/` 或用户通过 `LABEL_LOCAL_IMAGES_DIR` 指定的本地图片目录。
...
- YAML 默认生成到 `config/generated/<来源>_<国家>_<版本>_<数据集>.yaml`，所有目录参数都可以显式覆盖。
- EC2 训练目录同样按来源、国家、版本、数据集和运行名分层，例如 `models/ec2/s3/<国家>/<版本>/<数据集>/<EC2_RUN_NAME>/best.pt`。
...
- 扫描 `datasets/`、`datasets/excel/`、`datasets/local/`、`datasets/s3/`、历史兼容目录、`outputs/predict/`、`models/train/`、`artifacts/`、`data/samples/` 等本地目录，展示图片数量、训练集拆分、报告摘要和图片缩略图。
