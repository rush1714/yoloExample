...
当前通用类别定义已统一迁移到可持久化的 `config/label_categories.json`，默认只有 `LABEL_SET=general` 这一组“通用陈列类别”，里面的类别都可按业务需要自行增删改。历史品牌 OCR/YOLO-World 兼容流程继续使用 `config/brand_keywords.json` / `BRAND=<品牌>` 命令，不再把品牌集合写入默认通用类别配置。新的通用流程使用 `LABEL_SET=<类别列表>` 与 `LABELS=<类别1,类别2>` 选择单个或多个类别；控制台里如果要选择全部类别，直接在 `LABELS` 多选框中全选，不再提供 `All/all` 伪选项；命令行留空 `LABELS` 仍表示当前列表下所有启用类别，`all` 只作为历史兼容输入保留。控制台“类别管理”可维护类别列表，修改后写回 `config/label_categories.json` 并可提交 Git。
...
该流程独立于历史多品牌预标注，可用于 `纸尿裤`、`allround_purple` 或任意自定义单/多类别。

- 数据默认按国家、版本和类别隔离：`datasets/<国家>/<版本>/<数据集>/`，例如 `datasets/GH/v2026-09-18/allround_purple/` 或 `datasets/GH/v2026-09-18/general_all/`；Excel、本地目录、S3 只表示导入/访问方式，不再作为一级目录拆分同一数据集。
- 类别列表：`config/label_categories.json`，通过 `LABEL_SET` 选择列表，通过 `LABELS` 多选类别。
- 原图目录：`<数据集>/raw/images/`；用户通过 `LABEL_LOCAL_IMAGES_DIR` 指定本地源图片时，流程会先把图片沉淀到标准 `raw/images/`。
- Label Studio 导入 JSON：`<数据集>/label_studio/label_studio_import.json`。
- 不需要预标注时，使用 `1-label-excel-workflow-to-ls` 或 `1-label-local-workflow-to-ls` 直接导入原始图片和通用矩形框标签。
- 人工标注导出后转换为 `images/{train,val,test}` 和 `labels/{train,val,test}`；类别 ID 来自当前 `LABEL_SET/LABELS` 选择，单类别时可压缩为 `0`。
- 如果 `raw/images` 中包含大量未标注图片，标注后上云训练时可只上传已生成的 `images/{train,val,test}` 到 S3，然后用 `label-yolo-s3-to-ec2-manifest` 根据训练集和 S3 上传清单生成 `s3/metadata/ec2_image_manifest.json|csv`，避免上传无效原图。
- YAML 默认生成到 `config/generated/<国家>_<版本>_<数据集>.yaml`，所有目录参数都可以显式覆盖。
- EC2 训练目录同样按国家、版本、数据集和运行名分层，例如 `models/ec2/<国家>/<版本>/<数据集>/<EC2_RUN_NAME>/best.pt`。
...
- 项目已集成通用导出脚本 `scripts/export/export_model.py` 与 Make 命令 `make export-model`，支持自定义 `--model`、`--format` (`coreml`/`tflite`/`ncnn`/`onnx`/`engine`)、`--imgsz`、`--nms`（打包后处理简化端侧解析）、`--half` (FP16) 与 `--output-dir`，并已集成至本地 Web 控制台菜单。
...
├── raw/                         # 原始未标注图片；Excel 下载或本地目录沉淀后统一进入这里
├── ocr/                         # OCR 候选筛选结果
├── visual_prompts/              # YOLOE visual prompt 品牌参考包装图
├── pseudo/                      # YOLO-World/YOLOE 自动预标注结果
├── label_studio/                # LS 导入、标签配置、导出结果
├── s3/metadata/                 # S3 上传清单和 EC2 图片下载清单
├── images/                      # 人工复核后的正式训练图片
...
- 使用左侧多级菜单按“类别管理、图片来源/标注准备、Label Studio、本地 Mac 训练/推理、EC2 训练/评估/下载、维护工具、图片浏览”组织功能。
- “类别管理”读取和保存 `config/label_categories.json`，可维护多个类别列表；相关命令统一通过 `LABEL_SET` 选择类别列表，通过 `LABELS` 多选类别。
- 命令类功能可在左侧展开二级/三级子菜单；再次点击同一分组可折叠或展开，点击具体命令后右侧只展示该命令页面，包括说明、常用参数输入框/下拉框/多选框、命令预览、复制和执行按钮，未填写参数继续使用 Makefile 默认值。
- 参数输入会按 Make 参数名缓存到浏览器 `localStorage`，同名参数在不同命令页面共享；非敏感参数会同步写入 `web-console/state.json` 作为可提交的本地控制台状态，清空某个输入框会删除该参数缓存并恢复使用 Makefile 默认值，页面也提供“清空参数缓存”按钮。
- 通过 Node `spawn('make', [target, 'KEY=value'])` 执行白名单 Make 目标，并在页面展示实时日志。
- 扫描统一 `datasets/`、历史兼容目录、`outputs/predict/`、`models/train/`、`artifacts/`、`data/samples/` 等本地目录，展示图片数量、训练集拆分、报告摘要和图片缩略图。
...
- 通用导入脚本：`scripts/label_studio/generate_label_import.py`。
- Make 入口：`make 1-label-local-workflow-to-ls COUNTRY=<国家> DATA_VERSION=<版本> LABEL_SET=<类别列表> LABELS=<类别1,类别2> LABEL_LOCAL_IMAGES_DIR=<源图片目录>`。
- 人工标注后导出转换：`make 2-label-workflow-after-ls COUNTRY=<国家> DATA_VERSION=<版本> LABEL_SET=<类别列表> LABELS=<类别1,类别2> LS_PROJECT_ID=<项目ID>`。
- 旧 `1-local-dir-workflow-to-ls` / `2-local-dir-workflow-after-ls` 保留兼容，但推荐新项目使用通用 `label-*` 命令。
- 导入 JSON：`<LABEL_DATASET_ROOT>/label_studio/label_studio_import.json`。
- 标签配置：`<LABEL_DATASET_ROOT>/label_studio/label_config.xml`。
- YOLO 训练集：`<LABEL_DATASET_ROOT>/images/{train,val,test}` 和 `<LABEL_DATASET_ROOT>/labels/{train,val,test}`。
- YOLO YAML：`config/generated/<COUNTRY>_<DATA_VERSION>_<LABEL_DATASET_NAME>.yaml`。
- 本地 Web 控制台左侧菜单提供“类别管理”和“图片来源 / 标注准备 → 本地目录 → LS”分组，可维护类别并执行同样参数。
...
- S3 上传脚本：`scripts/s3/upload_images_to_s3.py`，通用流程输出 `datasets/<COUNTRY>/<DATA_VERSION>/<LABEL_DATASET_NAME>/s3/metadata/s3_images.json|csv` 和 `s3_download_urls.txt`；用户传入的 `S3_PREFIX` 是业务子目录，实际 S3 key 会自动归入 `yolo-training/<S3_PREFIX>/...`，若已传 `yolo-training/...` 则不会重复拼接；上传支持 `S3_UPLOAD_WORKERS` 并发和断点续传，已有成功记录且本地文件大小未变化的图片会跳过。`s3_images.json/csv` 是上传与 Label Studio 导入清单，不是 EC2 训练下载清单。
...
- 本地地址 LS 兼容转换脚本：`scripts/label_studio/export_local_s3_to_yolo.py`，用于“本地图片导入 LS 标注后，再把同一批图片上传 S3”的补充路径。它读取本地 LS 导出 JSON 和 `s3_images.json`，按 `local_path`、`relative_path`、`image_name` 匹配 S3 记录，生成 YOLO labels 与 `ec2_image_manifest.json|csv`，供 EC2 从 S3 下载训练图片。若同一批数据拆成多个 Label Studio 项目，可用 `LS_PROJECT_ID=21,20` 执行 `2-local-ls-s3-merge-workflow-after-ls`，先合并有效框任务再生成 S3 EC2 清单。
...
- EC2 批量推理验证使用 `make label-s3-ec2-predict-manifest`：输入可以是本地 `EC2_PREDICT_LOCAL_MANIFEST` 指向的 Excel/CSV/JSON/TXT 清单，也可以是 S3/HTTP/EC2 本地 `EC2_PREDICT_MANIFEST_SOURCE`；本地清单会先 rsync 到 EC2，EC2 再下载清单中的图片，用远端 `best.pt` 推理，最后把 `summary.json`、`summary.csv`、`source_image_uris.txt`、`uploaded_s3_uris.txt`、单图 JSON 和带框图片上传到 `EC2_PREDICT_OUTPUT_S3_URI` 指定的 S3 目录。如果推理已完成但 S3 上传因凭证或网络失败，可先修复 EC2 S3 凭证，再用 `make label-s3-ec2-upload-existing-predict-results` 只补传 EC2 work-dir 中已有结果，不重新推理。推理完成后可用 `make label-s3-ec2-download-predict-results` 把 S3 结果记录文本、结果文件和原始推理图片下载回本地标准目录：结果进入 `outputs/ec2_predict/<国家>/<版本>/<数据集>/<运行名>/`，原始推理图进入 `datasets/<国家>/<版本>/<数据集>/predict/images/<运行名>/`，记录和下载报告进入 `datasets/<国家>/<版本>/<数据集>/s3/metadata/`。
- EC2 命令仍默认 dry-run，只有显式传 `EC2_EXECUTE=1` 才会真正连接远端执行；EC2 访问 S3 推荐使用 Instance Profile/IAM Role 授权，避免在 EC2 上保存长期 Access Key。
...
# 4C. 如果本地地址 LS 标注拆在多个项目里，可用逗号分隔的 LS_PROJECT_ID 先合并再匹配 S3 清单
