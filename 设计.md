...
- EC2 编排脚本：`scripts/ec2/s3_workflow.py`，负责上传 labels/manifest/YAML、在 EC2 按 `ec2_image_manifest.json` 从 S3 下载图片、训练、评估归档、下载模型；`scripts/cloud/ec2_s3_workflow.py` 保留为兼容旧路径的入口。若误把上传阶段的 `s3_images.json` 当成 EC2 下载清单，脚本会提示先执行 `2-brand-s3-workflow-after-ls` 或 `local-ls-s3-to-yolo` 生成 EC2 清单。
