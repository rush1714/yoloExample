...
- EC2 批量推理验证使用 `make label-s3-ec2-predict-manifest`：输入可以是 S3/HTTP/EC2 本地 `txt`、`csv`、`json`、`xlsx` 图片地址清单，EC2 先下载清单和图片，再用远端 `best.pt` 推理，最后把 `summary.json`、`summary.csv`、单图 JSON 和带框图片上传到 `EC2_PREDICT_OUTPUT_S3_URI` 指定的 S3 目录。
