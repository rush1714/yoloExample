...
- 端到端组合：`make workflow-to-ls` 执行到 LS 导入；`make workflow-to-ls-llm` 使用 Ollama 本地大模型 OCR 后执行到 LS 导入；`make workflow-after-ls LS_PROJECT_ID=<项目ID>` 执行 LS 导出、训练和验证。
- 参数帮助：`make help` 会同时输出命令列表和常用参数默认值/可选值/效果说明；`make help-params` 只输出参数说明。重点参数包括 `OCR_WORKERS`、`OCR_FUZZY_THRESHOLD`、`LLM_OCR_MODEL`、`LLM_OCR_WORKERS`、`PSEUDO_BRAND_FILTER_ARGS`、`PSEUDO_NMS_IOU`、`PSEUDO_CONTAINMENT`、`PSEUDO_MAX_AREA_RATIO`、`TRAIN_EPOCHS`、`TRAIN_DEVICE`。
