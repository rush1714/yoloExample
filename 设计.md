...
当前类别定义来自 `data/brand_keywords.json`，并由 `make brand-yaml` 写入 `data/multibrand.yaml` / `data/multibrand_pseudo.yaml`。
...
- 脚本：`scripts/ocr/filter_brand_candidates.py`
- 品牌标识库：`data/brand_keywords.json`
- 输入：`datasets/multibrand/raw/images/`
- 输出目录：`datasets/multibrand/ocr/`
...
- OCR、预标注、Label Studio 多标签和 YOLO `names` 均以 `data/brand_keywords.json` 为统一类别来源。
- 预标注默认使用品牌库中全部启用品牌，按品牌映射到不同 YOLO class_id；如只调试单品牌，可传 `--brand-filter SOFTCARE`。
- 预标注会在同类别内做跨提示词 NMS、覆盖过滤和最大面积过滤，减少大框覆盖小框与重复框。
- `make brand-yaml` 根据品牌库生成 `data/multibrand.yaml` 和 `data/multibrand_pseudo.yaml` 的多品牌 `names`。
- 伪标注目录：`datasets/multibrand/pseudo/`
- 伪标注配置：`data/multibrand_pseudo.yaml`
...
- 导出训练集：人工复核完成后执行 `make step-5-export-ls-to-train LS_PROJECT_ID=<项目ID>`，导出 JSON 并转换为正式 `datasets/multibrand/images|labels/`。
- 端到端组合：`make workflow-to-ls` 执行到 LS 导入；`make workflow-after-ls LS_PROJECT_ID=<项目ID>` 执行 LS 导出、训练和验证。
- 参数帮助：`make help` 会同时输出命令列表和常用参数默认值/可选值/效果说明；`make help-params` 只输出参数说明。重点参数包括 `OCR_FUZZY_THRESHOLD`、`PSEUDO_BRAND_FILTER_ARGS`、`PSEUDO_NMS_IOU`、`PSEUDO_CONTAINMENT`、`PSEUDO_MAX_AREA_RATIO`、`TRAIN_EPOCHS`、`TRAIN_DEVICE`。
...
- `package` 能提高召回，但误检会增加；当前通过 `PSEUDO_NMS_IOU`、`PSEUDO_CONTAINMENT`、`PSEUDO_MAX_AREA_RATIO` 控制重复框和整图大框。
