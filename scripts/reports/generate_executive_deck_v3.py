"""
生成面向 COO / CTO 的 YOLO 门店陈列识别汇报 PPT (v3.0 升级版)。

该脚本基于真实业务演示文稿母版生成 16:9 高清商务汇报胶片，
特色与更新：
1. 完整继承森大与 IBM 官方母版（含左上角森大 Logo 与右上角 IBM Logo 及标准页脚）。
2. 封面采用纯白背景，符合母版规范。
3. 全局字号再次加大（最小字号 14pt，标题 24pt~28pt），适合高管投影汇报。
4. 泳道图重构第 4 页系统架构（业务数据层、智能初筛与标注层、自动化训练层、多端交付应用层）。
5. 优化第 6 页对比表格排版，去除多余冗余符号，边界完美对齐。
6. 加入 DeepSeek 多模态大模型价格对比与多梯次成本梯度（极低价/中档/旗舰），测算 2万张/天 与 10万张/天 规模。
7. 明确写明手机端运行 YOLO 的硬件制约（最低 3GB RAM + 64位 ARMv8 CPU，推荐 4G~6G RAM + NPU 加速）。
"""
...
OUTPUT_PPTX_PATH = PROJECT_ROOT / "docs" / "presentations" / "2026-09-23_YOLO门店陈列识别_COO_CTO汇报_v3.pptx"
...
def build_slide_1_cover(prs):
    ...
    p_sub.text = "聚焦重点 4 国（20,000张/天）与未来全量（100,000张/天）的技术选型、成本精算与终端落地 (v3.0)"
    p_sub.font.size = Pt(18)
    p_sub.font.name = FONT_HEADING
    p_sub.font.color.rgb = C_SENDA_BLUE
    p_sub.space_before = Pt(12)

    # 3 个核心决策摘要卡片 (大字体，纯净白色底)
    cards_data = [
...
def main():
    """PPT 生成主入口。"""
    print("正在生成面向 COO / CTO 的汇报 PPTX (v3.0)...")
    ...
    print(f"✅ PPT v3 生成成功：{OUTPUT_PPTX_PATH}")
