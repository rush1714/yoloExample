...
S3_ENDPOINT_URL ?=
S3_PUBLIC_BASE_URL ?=
# Label Studio 默认通过本地代理地址加载 S3 图片，避免依赖桶 CORS 配置。
S3_IMAGE_URL_MODE ?= proxy
...
brand-s3-proxy-start: ## 启动本地 S3 图片代理，供 Label Studio 加载私有桶图片
...
brand-s3-ls-apply: ls-db-check prepare-dirs ## 将 S3/proxy 图片任务导入 Label Studio
	cd $(LS_WORK_DIR) && printf 'exec(open("$(PROJECT_ROOT)/scripts/label_studio/apply_import.py", encoding="utf-8").read())\nexit()\n' | \
		...
		LS_PROJECT_DESCRIPTION='S3 图片单类别标注项目；图片通过 S3 URL 或本地 S3 代理加载。' \
...
1-brand-s3-workflow-to-ls: brand-s3-upload-images brand-s3-ls-import-json brand-s3-ls-apply ## 上传 S3、生成并导入 Label Studio
